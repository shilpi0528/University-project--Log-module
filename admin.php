...
<?php
    ...
    function read_log_file()   // Function to check for the checked boxes values
    {
        ...
        if(isset($_POST['submit']))
        {
          ...
          if (!empty($_POST['register']))
          {
             ...
             check_for_text($log,$dl_file,1);  // 1 for Login File
          }
    ...
    function check_for_text($log,$dl_file,$file)   // Function to check for entered Email ID and Date
    {
       $table ='';
       if ( !empty($_POST['email']))// && !empty($_POST['date']))
       {
          ...
          {
              $email = $_POST['email'];
              search($log,$email,$dl_file,$file,$table);
          }
       }
       else if (!empty($_POST['date']))
       {
          $date=$_POST['date'];
          search($log,$date,$dl_file,$file,$table);
       }
       else
       { 
          $match =0;
    ...
    function display ( $match, $line, $file, $table)
    {
        if ( $match == 1 )
        {
           if ( $file == 1 )
    ...
    function search( $log,$check,$dl_file,$file,$table )  // Function to find the data in the Log File
    {
       $line = fgets($log);
       $found =false;
       $match = 0;
